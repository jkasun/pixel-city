...
import type { FileNode, GitStatus } from './fileTypes'
import { GIT_STATUS_COLORS, getFolderColor, getFileIconData } from './fileTypes'
import { getGitStatusForPath } from './fileOperations'
import { setDraggedFilePath } from './dragState'
import { RefreshAltIcon } from '../icons/index.js'
...
function InlineNameInput({ parentPath, isFolder, onConfirm, onCancel, indent }: InlineInputProps) {
  ...
  const submit = () => {
    ...
    const fullPath = pathModule.join(parentPath, name)
...
function FileTreeNode({ node, style, dragHandle }: NodeRendererProps<FileNode>) {
  ...
  const gitStatus = (node.tree.props as any).gitStatus as Map<string, GitStatus> | undefined
...
export function FileTreePanel({
  ...
  return (
    <div className="files-sidebar flex flex-col min-w-[140px] max-w-[500px] bg-bg-card border-r border-border overflow-hidden" style={{ width: treeWidth }}>
      ...
      <div
        ...
        onDragOver={(e) => {
          // Allow native drops from Finder/OS
      ...
      >
        ...
        {/* Extra props (gitStatus, onNodeContextMenu) are accessed via `as any` in FileTreeNode */}
        <Tree<FileNode>
          data={treeData}
          openByDefault={false}
          width={treeWidth}
