...
import { RefreshAltIcon, SearchLargeIcon } from '../icons/index.js'
import { searchFiles, replaceInFile, replaceAllInFile } from './searchFiles'
import type { SearchResult, SearchFileResult, SearchMatch } from './searchFiles'
...
function FileGroup({ file, collapsed, onToggle, onMatchClick, onReplace, onReplaceAll, showReplace, replaceValue }: {
  ...
  const name = pathModule.basename(file.filePath)
...
export function SearchPanel({ projectCwd, width, onOpenFile, onRefreshTree }: SearchPanelProps) {
  ...
  }, [query, projectCwd, isRegex, isCaseSensitive, isWholeWord, includeGlob, excludeGlob])
  ...
  const handleReplaceSingle = useCallback((file: SearchFileResult, match: SearchMatch) => {
    if (replaceInFile(file.filePath, match.line, match.column, match.length, replaceValue)) {
      // Re-search to refresh results
      doSearch()
      onRefreshTree()
    }
  }, [replaceValue, doSearch, onRefreshTree])

  const handleReplaceAllInFile = useCallback((file: SearchFileResult) => {
    const count = replaceAllInFile(file.filePath, query, replaceValue, isRegex, isCaseSensitive, isWholeWord)
    if (count > 0) {
      doSearch()
      onRefreshTree()
    }
  }, [query, replaceValue, isRegex, isCaseSensitive, isWholeWord, doSearch, onRefreshTree])

  const handleReplaceAll = useCallback(() => {
    if (!result) return
    let totalReplaced = 0
    for (const file of result.files) {
      totalReplaced += replaceAllInFile(file.filePath, query, replaceValue, isRegex, isCaseSensitive, isWholeWord)
    }
    if (totalReplaced > 0) {
      doSearch()
      onRefreshTree()
    }
  }, [result, query, replaceValue, isRegex, isCaseSensitive, isWholeWord, doSearch, onRefreshTree])
  ...
  return (
    <div className="files-sidebar flex flex-col min-w-[140px] max-w-[500px] bg-bg-card border-r border-border overflow-hidden" style={{ width }}>
      {/* Header */}
      ...
      {/* Results */}
      <div className="flex-1 overflow-y-auto [&::-webkit-scrollbar]:w-[5px] [&::-webkit-scrollbar-thumb]:bg-border [&::-webkit-scrollbar-thumb]:rounded-[3px]">
        {/* Linear loading bar */}
        {searching && (
        ...
        {!searching && result && result.totalMatches === 0 && query.trim() && (
        ...
        {!searching && result && result.files.map(file => (
          <FileGroup
            key={file.filePath}
            file={file}
