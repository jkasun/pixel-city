...
install_mac() {
  ...
  info "Mounting DMG"
  mount_out=$(hdiutil attach "${tmpdir}/${asset}" -nobrowse -quiet)
  mount_point=$(printf '%s\n' "$mount_out" | awk '/\/Volumes\// { for (i=3;i<=NF;i++) printf "%s ", $i; print "" }' | sed 's/ *$//')
...
install_linux() {
  ...
  version=$(resolve_version) || exit 1
  asset="PixelCity-${version}-linux-${arch}.AppImage"
  url="${BASE_URL}/${asset}"
