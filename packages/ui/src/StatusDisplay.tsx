...
// Detect if text is already a Claude PTY spinner like "Incubating… (48s · ↓ 883 tokens)"
...
export function StatusDisplay({ text, style, startedAt, tokens }: StatusDisplayProps) {
  ...
  // Build the meta string: (12s · ↓ 883 tokens)
  let meta = ''
  if (isActive && elapsed >= 1000) {
    const parts = [formatElapsed(elapsed)]
    if (tokens != null && tokens > 0) parts.push(`↓ ${tokens} tokens`)
    meta = ` (${parts.join(' · ')})`
  }
