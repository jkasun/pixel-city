...
// ── Wake / Sleep ─────────────────────────────────────────────
/** Wake-in alpha lerp duration (asleep → awake). */
export const WAKE_IN_DURATION_SEC = 0.6
/** Sleep-out alpha lerp duration (awake → asleep). */
export const SLEEP_OUT_DURATION_SEC = 0.4
/** Asleep ghost alpha. Bumped from 0.35 → 0.6 so ghosts read as present-but-dormant
 *  instead of "barely visible"; still clearly distinguishable from full-color live chars. */
export const ASLEEP_ALPHA = 0.6
/** Z-glyph fade/drift loop period. */
export const Z_GLYPH_PERIOD_SEC = 2.4
/** Vertical drift amplitude of Z-glyph in sprite pixels. */
export const Z_GLYPH_DRIFT_PX = 2
/** Z-glyph alpha swing: alpha = ALPHA_MIN + ALPHA_RANGE * (sin(...) + 1) / 2
 *  Scaled with ASLEEP_ALPHA bump (0.35→0.6) so the pulse stays balanced against
 *  the brighter ghost body. Capped so peak ≤ 1.0. */
export const Z_GLYPH_ALPHA_MIN = 0.6
export const Z_GLYPH_ALPHA_RANGE = 0.4
/** Pixels above character's head where glyph is anchored (sprite-space). */
export const Z_GLYPH_OFFSET_Y_PX = 4
/** Minimum ms between wake/sleep animation dispatches from the queue. */
export const WAKE_MIN_STAGGER_MS = 250
...
// ── Vehicles ────────────────────────────────────────────────
export const VEHICLE_SPAWN_INTERVAL_SEC = 3.0
export const VEHICLE_MAX_COUNT = 20
export const VEHICLE_COLLISION_DISTANCE_TILES = 1.2
export const VEHICLE_DEFAULT_SPEED_MIN = 1.5 // tiles per second
export const VEHICLE_DEFAULT_SPEED_MAX = 3.5
