...
import { randomName } from './nameData.js'
import {
...
import { hashToUnit } from './wakeAnimationQueue.js'
...
export function createCharacter(
  ...
  return {
    ...
    glyphPhase: hashToUnit(id) * Z_GLYPH_PERIOD_SEC,
  }
}
...
export function updateCharacter(
  ...
  switch (ch.state) {
    ...
    case CharacterState.IDLE: {
      ...
      // If became active, pathfind to seat (skip if user-commanded walk)
      if (ch.isActive && !ch.userWalk) {
        if (!ch.seatId) {
    ...
    case CharacterState.WALK: {
      ...
      if (ch.path.length === 0) {
        ...
        ch.x = center.x
        ch.y = center.y
        const wasUserWalk = ch.userWalk
        // Keep userWalk flag — cleared by sendToSeat/setAgentActive

        if (ch.isActive && !wasUserWalk) {
          if (!ch.seatId) {
        ...
        } else if (ch.isActive && wasUserWalk) {
          // User-commanded walk completed — stay idle at destination
          ch.state = CharacterState.IDLE
          ch.wanderTimer = randomRange(WANDER_PAUSE_MIN_SEC, WANDER_PAUSE_MAX_SEC)
        } else {
      ...
      // If became active while wandering, repath to seat (skip if user-commanded walk)
      if (ch.isActive && ch.seatId && !ch.userWalk) {
        const seat = seats.get(ch.seatId)
        if (seat) {
