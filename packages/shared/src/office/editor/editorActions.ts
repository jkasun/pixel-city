...
export function placeFurniture(layout: OfficeLayout, item: PlacedFurniture): OfficeLayout {
  ...
  return { ...layout, furniture: [...furniture, item] }
}
...
export function removeFurniture(layout: OfficeLayout, uid: string): OfficeLayout {
  const furniture = ensureFurnitureArray(layout.furniture)
  const filtered = furniture.filter((f) => f.uid !== uid)
  if (filtered.length === furniture.length) return layout
...
export function moveFurniture(layout: OfficeLayout, uid: string, newCol: number, newRow: number): OfficeLayout {
  ...
  return {
    ...layout,
    furniture: furniture.map((f) => (f.uid === uid ? { ...f, col: newCol, row: newRow } : f)),
  }
}
...
export function rotateFurniture(layout: OfficeLayout, uid: string, direction: 'cw' | 'ccw'): OfficeLayout {
  ...
  return {
    ...layout,
    furniture: furniture.map((f) => (f.uid === uid ? { ...f, type: newType } : f)),
  }
}
...
export function canPlaceFurniture(
  ...
  const furniture = ensureFurnitureArray(layout.furniture)
  const occupied = getPlacementBlockedTiles(furniture, excludeUid)
