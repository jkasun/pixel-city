...
export const AGENT_MODELS = [
  { id: 'claude-sonnet-4-6', label: 'Sonnet', color: '#5ac8e8', bg: 'rgba(90, 200, 232, 0.15)' },
  { id: 'claude-opus-4-6', label: 'Opus', color: '#c87aff', bg: 'rgba(200, 122, 255, 0.15)' },
]
...
export interface CharacterPickerProps {
  onPick: (palette: number, model: string, customName: string, initialMessage?: string) => void
  onClose: () => void
}

export function CharacterPicker({ onPick, onClose }: CharacterPickerProps) {
  ...
  return (
    <div
      data-testid="office-character-picker"
      style={{
        position: 'absolute',
        bottom: 52,
        left: 10,
    ...
    >
      ...
      <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
        ...
        {previews.map((src, i) => (
          <button
            key={i}
            data-testid={`office-character-option-${i}`}
        ...
        <button
          data-testid="office-character-close-btn"
          onClick={onClose}
      ...
      <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
        ...
        <input
          data-testid="office-character-name-input"
          type="text"
          value={customName}
      ...
      <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
        ...
        <input
          data-testid="office-character-message-input"
          type="text"
          ...
          onKeyDown={e => {
            if (e.key === 'Enter' && selectedPalette !== null) {
              onPick(selectedPalette, AGENT_MODELS[0].id, customName, initialMessage || undefined)
            }
          }}
        />
      </div>
      ...
      <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
        ...
        {AGENT_MODELS.map(m => (
          <button
            key={m.id}
            data-testid={`office-character-model-${m.id}`}
